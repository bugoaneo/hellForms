const CustomForm = {
    ...
    fixupUploads: function (e) {
        //$('.fileuploader form, .fileuploader__multiple form').hide();
        let btn = $("button[type='submit']", $(e).closest(".block__content"));
    ...
    customFormSendResult: function (data) {
        ...
        if (data.isSuccess) {
            ...
            if (data.extraData !== null) {
                for (var key in data.extraData) {
                    if (key !== "") {
                        $("span[data-valmsg-for='" + key + "']", $(this)).text(data.extraData[key].errors[0].errorMessage).removeClass().addClass("field-validation-error");
                        $("[name='" + key + "']", $(this)).removeClass("valid").addClass("input-validation-error");
                    } else {
                        $(".validation-summary-errors ul", $(this)).append("<li>" + data.extraData[key].errors[0].errorMessage + "</li>");
                    }
                }
            }
    ...
    //customFormFailure: function (data) {
    ...
    //customFormFeedbackBegin: function () {
    //    $(".validation-summary-errors div", $(this)).html("");
    //    $(".validation-summary-errors ul", $(this)).html("");
    //    $('button', $(this)).prop('disabled', true);
    //},

    //customFormFeedbackComplete: function () {
    //    //$('button', $(this)).prop('disabled', false);
    //    //grecaptcha.reset();
    //}
};
...
function executeCaptcha() {
    ...
    grecaptcha.execute('6LfgAqYpAAAAACGFBVfjEz6GZxIiHt26dj4KU4w-', { action: 'submit' })
        .then(function (token) {

            var captchaTokens = document.querySelectorAll('#g-recaptcha-response');
            for (var i = 0; i < captchaTokens.length; i++) {
                captchaTokens[i].value = token;
            }
        });
}
...
$(function () {
    ...
    $("form.custom-form").submit(function (event) {
        var $form = $(this);
        if ($form.valid()) {
            //var fileUpload = $("form.custom-form input[type=file]").get(0);
            var fileUpload = $($form.get(0)).find("input[type=file]");
            var fileData = new FormData();

            for (var item of fileUpload) {
            ...
            for (var t in formValue) {
                fileData.append(t, formValue[t]);
            }
            $('button', $(this)).prop('disabled', true);
            ...
            $.ajax({
                ...
                CustomForm.customFormSendResult.call($form, data);
            });
        }
